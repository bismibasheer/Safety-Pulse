...
def Viewcomplaints(request):
    if "mid" in request.session:
        ...
        replycom=tbl_Complaint.objects.filter(complaint_status=1)
        if request.method=='POST':
...
def repliedcomplaint(request):
    if "mid" in request.session:
        complaint=tbl_Complaint.objects.filter(complaint_status=0)
        replycom=tbl_Complaint.objects.filter(complaint_status__gt=0)
        if request.method=='POST':
...
def multiple_violations(request):
    ...
    for i in vehicle:
        repeat = tbl_Complaint.objects.filter(vehicle_number=i.id, complaint_status=2).count()
        if repeat > 3:
            cmp.append(i.id)
    ...
    for vehicle in vehicledata:
        ...
        complaints = tbl_Complaint.objects.filter(vehicle_number=vehicle.id, complaint_status__lte=2)
...
def report(request):
    ...
    all_complaints = tbl_Complaint.objects.filter(User__place__id=placeid).order_by('complaint_date')
